...
class Surgery(object):
    ...
    def set_weight(self, weight, data_numpy=None, all_ones=False, all_zeros=False):
        # NOTE: weight can be stored in human readable fields(float_data, int32_data, string_data, ...)
        ...
        if data_numpy is not None:
            ...
            if all_ones:
                ...
                # set weight as random or nan elements
                # wr = numpy_helper.to_array(weight)
                # wn = np.empty_like(wr, dtype=object)
                # set weight as empty tensor data
                wn = np.empty(shape=(0, 3))
